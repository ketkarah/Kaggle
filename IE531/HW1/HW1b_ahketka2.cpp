...
{
	double t;
	// randomly assign values for prob1, prob2 and prob3
	// such that prob1 + prob2 + prob3 = 1
	srand(time(NULL));
	prob1 = rand() % 100;
	t = 100 - prob1;
	prob2 = rand() % int(t);
...
int three_event_generator()
{
	double random =(rand() % 100 + 1) / (double)100;
	if (random <= prob1) return EVENT1;
	random -= prob1;
	if (random <= prob2) return EVENT2;
	random -= prob2;
	if (random <= prob3) return EVENT3;
	random -= prob3;
}
...
int simulate_fair_coin_toss_from_three_event_generator()
{
	...
	eve1 = three_event_generator();
	eve2 = three_event_generator();
	eve3 = three_event_generator();
	add = eve1 + eve2 + eve3;
	...
	else simulate_fair_coin_toss_from_three_event_generator();
}
...
// 0 and 2^32-1.
int get_ones_and_zeros()
{
	int out1 = simulate_fair_coin_toss_from_three_event_generator();
	int out2 = simulate_fair_coin_toss_from_three_event_generator();
	if (out1 == 1 && out2 == 0) {
		return 1;
	}
...
double get_uniform_from_three_event_generator()
{
	int arr[32];
	double sum = 0.0;
	for (int i = 0; i < 32; i++) {
		arr[i] = get_ones_and_zeros();
	}
	for (int j = 0; j < 32; j++) {
		sum += (arr[j] * pow(2, j));
	}
...
double get_exp_from_unfair_coin(double lambda)
{
	return ((-1.0 / lambda)*log(get_uniform_from_three_event_generator()));
}

// This generates i.i.d. rv.s that are unit-normal distributed using an unfair-coin
// (I am using the Box-Muller method here)
double get_gaussian_from_unfair_coin()
{
	return (sqrt(-2.0*log(get_uniform_from_three_event_generator()))*cos(2 * 3.141592654*get_uniform_from_three_event_generator()));
}
...
int main(int argc, char* argv[])
{
	...
	{
		double y = get_exp_from_unfair_coin(lambda);
		for (int j = 0; j < 100; j++)
			if (y < ((double)j / 10))
				count[j]++;
	}
	...
	for (int i = 0; i < no_of_trials; i++)
	{
		double y = get_gaussian_from_unfair_coin();
		for (int j = 0; j < 100; j++)
			if (y < ((float)(j - 50) / 10))
				count[j]++;
	}
